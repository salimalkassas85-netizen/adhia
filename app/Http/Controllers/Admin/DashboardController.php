...
class DashboardController extends Controller
{
    public function __invoke(AdminAreaScope $scope)
    {
        ...
        return view('admin.dashboard', [
            ...
            'assignedRequests' => (clone $requests)->where('status', 'assigned')->count(),
            'deliveredRequests' => (clone $requests)->where('status', 'delivered')->count(),
            'failedRequests' => (clone $requests)->where('status', 'failed')->count(),
            'totalDonations' => (clone $donations)->count(),
            'pendingDonations' => (clone $donations)->where('status', 'pending')->count(),
            'confirmedDonations' => (clone $donations)->whereIn('status', ['confirmed', 'received', 'allocated', 'in_distribution'])->count(),
